...
// Add new task event listener for Academic Tasks
newTaskForm.addEventListener("submit", (e) => {
    ...
    if (taskDescription && dueDate) {
        // Save task to Firebase under "AcademicTasks"
        const tasksRef = ref(db, "AcademicTasks");
...
// Display tasks and set up edit/delete functionality for Academic Tasks
const displayTasks = () => {
    const tasksRef = ref(db, "AcademicTasks");
    onValue(tasksRef, (snapshot) => {
        ...
        snapshot.forEach((childSnapshot) => {
            ...
            saveButton.addEventListener("click", () => {
                ...
                const taskRef = ref(db, `AcademicTasks/${taskId}`);
                update(taskRef, {
...
// Check due dates for Academic Tasks periodically
const checkDueDates = () => {
    const tasksRef = ref(db, "AcademicTasks");
    onValue(tasksRef, (snapshot) => {
        ...
        snapshot.forEach((childSnapshot) => {
            ...
            if (
                ...
                alert("Your academic task is due today!");
            }
        });
    });
};
